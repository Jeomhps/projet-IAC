...
func (h *Machines) List(c *gin.Context) {
	...
	for _, m := range ms {
		...
		if admin {
			item["host"] = m.Host; item["port"] = m.Port; item["user"] = m.User
			item["reserved_by"] = m.ReservedBy; item["enabled"] = m.Enabled; item["online"] = m.Online
...
func (h *Machines) Update(c *gin.Context) {
	...
	for _, f := range []string{"host","port","user","password","enabled","online","name"} {
		if v, ok := in[f]; ok {
			...
			if f == "port" {
				switch vv := v.(type) {
				case float64: args = append(args, int(vv))
				case string:
					if n, err := strconv.Atoi(vv); err == nil { args = append(args, n) } else { args = append(args, vv) }
	...
	if len(fields) == 0 {
		c.JSON(http.StatusBadRequest, gin.H{"error":"invalid_request","message":"no fields"}); return
	}
...
func (h *Machines) Delete(c *gin.Context) {
	...
	if reserved {
		c.JSON(http.StatusBadRequest, gin.H{"error":"bad_request","message":"Machine is currently reserved; release first"}); return
	}
